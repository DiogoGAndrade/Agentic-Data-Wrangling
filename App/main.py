...
from engine.actions import (fix_column_names, cast_type, handle_missing,
                             normalize_text, deduplicate, drop_column,
                             encode_categorical, remove_outliers)
...
with st.sidebar:
    ...
    base_url = st.text_input(
        "Ollama server URL", "http://localhost:11434", key="set_ollama_url",
        help="Leave unchanged unless Ollama is running on a different machine.")

    # Recommended models (label → ollama tag). The user can run any of these,
    # but only AFTER pulling them with `ollama pull <tag>`.
    LLM_OPTIONS = {
        "Qwen 2.5 3B  (fastest, ~2 GB RAM)":        "qwen2.5:3b-instruct",
        "Llama 3.2 3B  (fast, ~2 GB RAM)":           "llama3.2:3b",
        "Mistral 7B  (balanced, ~5 GB RAM)":         "mistral:7b",
        "Qwen 2.5 7B  (balanced, ~5 GB RAM)":        "qwen2.5:7b",
        "Llama 3.1 8B  (balanced, ~6 GB RAM)":       "llama3.1:8b",
        "Gemma 2 9B  (higher quality, ~7 GB RAM)":   "gemma2:9b",
        "Mistral NeMo 12B  (high quality, ~9 GB RAM)": "mistral-nemo:12b",
        "Qwen 2.5 14B  (best quality, ~11 GB RAM)":  "qwen2.5:14b",
        "Other… (type an exact Ollama tag)":            "__custom__",
    }

    # Ask the local Ollama which models are actually installed.
    _installed = OllamaClient(base_url=base_url).list_models()

    if _installed:
        # Build a list that surfaces installed models first and marks each
        # recommended model as installed or not. This removes the trap of
        # picking a model that isn't downloaded.
        def _is_installed(tag):
            # Ollama tags often carry a ":latest"/version suffix; match loosely.
            base = tag.split(":")[0]
            return any(m == tag or m.split(":")[0] == base for m in _installed)

        labelled = {}
        for lbl, tag in LLM_OPTIONS.items():
            if tag == "__custom__":
                labelled[lbl] = tag
            elif _is_installed(tag):
                labelled[f"✓ {lbl}"] = tag
            else:
                labelled[f"⬇ {lbl} (not installed)"] = tag
        # Also offer any installed model that isn't in our recommended list.
        _known = {t for t in LLM_OPTIONS.values() if t != "__custom__"}
        for m in _installed:
            if m.split(":")[0] not in {t.split(":")[0] for t in _known}:
                labelled[f"✓ {m}  (installed)"] = m

        llm_label = st.selectbox(
            "Choose AI model", list(labelled.keys()), index=0, key="set_llm",
            help="✓ = installed and ready. ⬇ = recommended but not yet "
                 "downloaded; run `ollama pull <tag>` first.")
        chosen = labelled[llm_label]
        if chosen == "__custom__":
            model = st.text_input(
                "Exact Ollama tag", "qwen2.5:3b-instruct", key="set_custom_tag",
                help="Advanced: type the exact tag of any model you have pulled, "
                     "including a specific quantisation (e.g. qwen2.5:3b-instruct-q4_0). "
                     "Models you already have are listed above automatically; you "
                     "only need this for an exact tag not shown there.")
        else:
            model = chosen
            if llm_label.startswith("⬇"):
                st.warning(f"`{model}` is not installed. Run this in a terminal "
                           f"first:\n\n`ollama pull {model}`")
    else:
        # Ollama not reachable: fall back to the static list and warn.
        st.warning("Could not reach Ollama to list installed models. "
                   "Make sure Ollama is running, then reload. "
                   "You can still pick a model below.")
        llm_label = st.selectbox(
            "Choose AI model", list(LLM_OPTIONS.keys()), index=0, key="set_llm",
            help="Models must be downloaded first with `ollama pull <tag>`.")
        chosen = LLM_OPTIONS[llm_label]
        model = (st.text_input("Custom model tag", "qwen2.5:3b-instruct",
                               key="set_custom_tag")
                 if chosen == "__custom__" else chosen)
...
if uploaded is not None:
    try:
        df0 = _load_csv(uploaded, sep=sep, enc_choice=enc_choice, na_values=na_values)
...
# Session state init
_defaults = {
    "df_original": df0.copy(), "df_current": df0.copy(),
    "provenance": ProvenanceLog(), "last_plan": None,
    "uploaded_name": getattr(uploaded, "name", None),
    "step": 1, "execution_done": False,
}
...
if st.session_state.step < 2:
    ...
    with col_btn:
        ...
        gen_btn = st.button("Propose plan (review first)", type="primary",
                            use_container_width=True,
                            help="Ask the AI to draft a cleaning plan. "
                                 "You approve each step in Step 3 before anything runs.")
        auto_btn = st.button("Auto-clean (full autonomy)", use_container_width=True,
                             help="The AI proposes a plan AND the system applies "
                                  "every step automatically, then hands you the "
                                  "cleaned dataset. Skips manual review; best for "
                                  "quick, hands-off cleaning.")
...
if st.session_state.step < 4 and not st.session_state.execution_done:
    ...
    plan_dict = st.session_state.last_plan.model_dump() if st.session_state.last_plan else {}
