"""
Run all C3 (context-aware) experiments.

This script iterates over every (dataset, ml_model) combination,
uses the matching user-context JSON, and calls prepare_conditions
followed by run_experiments for each.

Usage:
    python -m evaluation.run_c3_all --llm-tag qwen2.5_3b --ollama-model "qwen2.5:3b"

To run ALL 8 LLMs sequentially:
    python -m evaluation.run_c3_all --all-llms
"""
...
# LLMs to test (same as Phase A + B)
...
# Dataset × ML-model combinations (must match context file names)
...
# Phase B: test/generalization datasets
PHASE_B_COMBOS: Dict[str, List[str]] = {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run all C3 context-aware experiments.")
    ...
    parser.add_argument("--skip-prepare", action="store_true",
                        help="Skip plan generation (use existing C3 plans).")
    ...
    for llm_tag, ollama_model in llms_to_run:
        ...
        # Merge Phase A and Phase B combos
        all_combos = {**PHASE_A_COMBOS, **PHASE_B_COMBOS}
        for dataset_id, ml_models in all_combos.items():
            ...
            for ml_model in ml_models:
                ...
                # Step 1: Generate C3 plan (one per dataset×llm, reuses if same)
                if not args.skip_prepare:
                    # The plan tag encodes both the LLM and the ML model context
    ...
    # Summary
    print(f"\n{'='*70}")
    print(f"C3 EXPERIMENT RUN COMPLETE")
    print(f"{'='*70}")
    ...
    print(f"       to merge C3 results into the MASTER table.")
